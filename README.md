...
以下是**實測數字**，不是設計時的預期。兩組資料要分清楚：
12 筆是 demo（樣本太小，只看趨勢），**結論一律以 50 筆評測集為準**。

50 筆評測集（`poc/run_eval.py`，350 次 rubric 檢查，雜訊底線 0.29pp）：

| 指標 | v0 | v1 | v2 | v3 | 實測歸因 |
|---|---|---|---|---|---|
| 標題長度合規 | 68% | **100%** | 100% | 100% | v1 |
| 規格完整覆蓋 | 92% | **100%** | 100% | 100% | v1 |
| 法規禁詞 0 命中 | 76% | 96% | **100%** | 100% | v1（v2 收尾）|
| rubric 通過率 | 68.4% | 79.1% | 86.3% | 86.6% | v2 |
| 可直接上架 | 14% | 42% | **72%** | 68% | v2 |
| 可機器讀取 | 0% | 0% | 0% | **100%** | v3 |

**要誠實講的兩件事：**

- **v2 → v3 的 rubric 只差 0.3pp，在雜訊內。** 不能宣稱 v3 讓文案變好。
  v3 的價值在最後一欄：**不是讓文案變好，是讓文案變得可用。**
- **法規那欄 v1 就做掉大部分**（76→96），v2 只是收尾（96→100）。
  但 4% 不是 0% —— prompt 只能降低機率，規則層才能給保證。

12 筆 demo（notebook 內，`OFFLINE_MODE` 重播）：

| 指標 | v0 | v1 | v2 | v3 |
|---|---|---|---|---|
| 標題長度合規 | 83% | 100% | 100% | 100% |
| 法規禁詞 0 命中 | 75% | 92% | 100% | 100% |
| 可機器讀取 | 0% | 0% | 0% | 100% |
| rubric 通過率 | 78.5% | **74.0%** | 80.9% | 83.3% |
| 受評筆數 | **8** | 11 | 12 | 12 |

> ⚠️ 注意 12 筆的 rubric 那列**不是遞增的**：v0 的 78.5% 比 v1 高。
> 那不是 v0 比較好 —— 是 v0 有 4 筆被規則層擋下、根本沒送評分，
> **分母不同（8 vs 11）就不能直接比**。這正是表上要印「受評筆數」的原因，
> 也是 12 筆樣本不足以下結論的實例。
