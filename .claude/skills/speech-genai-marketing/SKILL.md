...
- v0～v3 那四頁（第 9–12 頁）是同一個版型：**左欄＝這一版 prompt 加了什麼，
  右欄＝它的產出**。兩欄的文字都抄自 `poc/data/`（prompt 抄 `src/prompts.py`
  組出來的字串，產出抄 `demo_outputs.json` 重播的真實輸出），商品固定 HB-1001。
  改 prompt 或重錄 fixtures 之後要回來對一次。
- **這四頁一個通過率都不放。** 評測要到第 15 頁三層金字塔才登場，提早給百分比
  聽眾沒有框架可以接 —— 這是實際被聽眾回饋過的問題，不要「順手補個數字更有力」。
- `layout_code()` 會把行內空白換成不斷行空白：SVG 預設把連續空白摺成一個，
  縮排與欄位對齊會整個垮掉。貼 JSON／schema 時縮排就是可讀性，不要改回去。
