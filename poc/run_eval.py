"""在評測集上跑完整流程，產出可引用的統計結論。

## 這支跟 notebook 的分工

**兩邊用同一份資料**：`data/eval_products.json` 共 200 筆。
notebook 取前 50 筆（`build_notebook.DEMO_N`），這支預設全跑，
要對齊台上那張表就下 `--limit 50`。

- **notebook**：台上跑的 demo。要快、要看得懂、要能內嵌成單一檔案。
- **這支**：離線跑的評測，可以往上加樣本數，回答「這個差距是真的還是雜訊」。

早期 notebook 用手寫的 12 筆、評測用另外 200 筆，結果台上得解釋
「這張表是 12 筆、但結論來自 50 筆」—— 多一層說明就多一分不被信任。
統一之後，台上那張表就是結論本身。

50 筆 × 7 條 rubric = 350 次檢查，雜訊底線 0.29 個百分點。
但那是**解析度不是顯著性** —— 配對比較的 95% 信賴區間寬達 ±5pp，
差了一個數量級。要下更細的結論就把 `--limit` 往上加。
...
# 因為瓶頸是等待回應而非 CPU（2026-08-13、單一專案預設配額、零失敗）：
