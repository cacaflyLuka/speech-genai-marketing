...
前面每一節都有自己的表格，適合邊做邊解釋。但**看完之後沒有一個地方能一眼看完整體** ——
決策者要的正是後者：哪一版能上線、差距是不是真的、這件事要花多少錢。

這一頁**不產生任何新數字**，全部來自上面已經算過的結果，只是換一種呈現。
若這裡的數字和上面的表格對不上，那是 bug，不是「圖表的呈現差異」。

四個看板數字是結論，下面三區是支撐它的證據：規則層（免費、確定性）、
評審層與**信賴區間**、以及成本組成。

> 右上那張信賴區間圖是整頁最重要的一張。長條圖只能顯示「差多少」，
> 顯示不了「這個差距可不可信」—— 區間跨過 0 就不能宣稱誰比較好。
"""))
    cells.append(code(module_source("dashboard")))
    cells.append(code("""
fig = render_dashboard(
    rule_results,
    summarize_rubrics(rubric_reports, total_items=len(PRODUCTS)),
    rubric_reports,
    ledger,
    insights=globals().get('insights'),   # §4 被跳過時就不畫那一格
)
fig.savefig('evaluation_overview.png', dpi=150, facecolor=fig.get_facecolor(),
            bbox_inches='tight')
print('已存成 evaluation_overview.png —— 可以直接貼進報告或簡報')
"""))
    cells.extend(cue("""
**41:30 — 捲到儀表板，這是最後一張畫面。**

不要逐格解說，只講四個看板數字，然後指右上那張圖：

「這張是整場的重點 —— 四個版本之間，只有一個差距是統計上成立的。
另外兩個區間跨過 0，代表**在這個樣本量下分不出差別**，不是一樣好。」

如果有人要這張圖，它已經存成 PNG 了。
"""))
