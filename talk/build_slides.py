        ...
        for i, ln in enumerate(panel["lines"]):
            if i in panel.get("highlight", ()):
                body.append(rect(x + 12, y - 16, col_w - 24, 24, fill=hl,
                                 stroke="none", r=4))
            # SVG 會把連續空白摺成一個，縮排和欄位對齊會整個垮掉 ——
            # 這裡是貼 prompt 與 JSON 的地方，縮排就是可讀性本身，所以換成
            # 不斷行空白。等寬字型下兩者寬度一樣，看不出差別。
            body.append(text(x + 20, y, ln.replace(" ", " "),
                             size=15, color=INK_SOFT, font=MONO))
            y += 24
    if note:
        body += [
...
def build_slides() -> list[tuple[str, str, str]]:
    ...
    # ------------------------------------------------------------- 轉 1
    #
    # v0～v3 這四頁是同一個版型：**左欄是 prompt 這一版加了什麼，右欄是它的產出**。
    # 四頁的商品固定是 HB-1001（金盞花萃取葉黃素膠囊 60粒），左右兩欄的文字都
    # 直接抄自 poc/data/ ——  prompt 抄 src/prompts.py 組出來的字串，產出抄
    # demo_outputs.json 重播出來的真實輸出。這四頁**刻意不放任何通過率**：
    # 評測要到後面三層金字塔那段才登場，提早給數字聽眾接不上。
    gen("prompt-v0.svg", "v0：什麼都不給",
        "最常見的第一版 prompt，以及它實際產生的東西。",
        layout_code(
            title="v0　什麼都不給",
            subtitle="大多數人第一次寫的 prompt：一句話加商品資料",
            panels=[
                {"label": "v0 prompt", "tone": "gray",
                 "lines": [
                     "幫我寫這個商品的電商文案。",
                     "",
                     "商品：晨光研選 金盞花萃取葉黃素膠囊 60粒",
                     "品類：保健食品 / 眼部保養",
                     "規格：",
                     "  - 淨含量：60粒 / 盒",
            ...
            note="什麼都沒說，模型就自己決定 —— 可能會踩到法規。",
        ))

    gen("prompt-v1.svg", "v1：加通路約束",
        "v1 加入角色、受眾、字數上限與必含規格。",
        layout_code(
            title="v1　加通路約束",
            ...
            panels=[
                ...
                {"label": "產出", "tone": "gray",
                 "lines": [
                     ...
                     "",
                     "### 賣點（4 條，每條不超過 30 字）",
                     "1. 足量高濃度：每粒含游離型葉黃素 30mg⋯⋯",
                     "2. 黃金比例：國際認證 5:1 比例，搭配⋯⋯",
                     "",
                     "### SEO 描述（不超過 120 字）",
                     "專為長時間使用螢幕的 25-45 歲上班族⋯⋯",
                 ]},
            ],
            note="字數與規格是能自動檢查的條件。",
        ))
    ...
    gen("prompt-v2.svg", "v2：加法規與語調",
        ...
        layout_code(
            ...
            panels=[
                {"label": "v2 新增（標色部分）", "tone": "orange",
                 "lines": [
                     "【法規限制 —— 違反會被開罰】",
                     "本商品受《食品安全衛生管理法》第 28 條規範。",
                     "不可出現：治療、改善視力、療效、根治⋯⋯",
                     "（依商品類別帶入，這一類共 93 個禁詞）",
                     "",
                     "【品牌語調】",
                     "溫和、專業、不誇大，訴求日常持續補充",
                     "以下是本品牌既有的合規文案，請模仿：",
                     "  晨光研選 緩釋B群錠 90錠 每日一錠 全素可食",
                     "  晨光研選 甘胺酸鎂錠 120錠 睡前補充",
                 ],
                 "highlight": (2, 8, 9)},
                {"label": "產出", "tone": "gray",
                 "lines": [
                     "### 商品標題",
                     "晨光研選 金盞花游離型葉黃素膠囊 30mg 60粒",
                     "",
                     "### 賣點（4 條）",
                     "1. 足量游離型葉黃素 30mg，吸收率佳。",
                     "2. 採用葉黃素：玉米黃素 5:1 黃金比例。",
                     "3. 專為長時間使用螢幕的上班族設計。",
                     "4. 每盒 60粒，每日一粒，持續補充兩個月。",
                     "",
                     "### 標籤",
                     "葉黃素、游離型葉黃素、眼部保養、30mg",
                 ],
                 "highlight": (1,)},
            ],
            note="禁詞給清單，語調給範例 —— 都是直接給，不是用形容的。",
        ))
    ...
    gen("prompt-v3.svg", "v3：結構化輸出",
        ...
        layout_code(
            title="v3　結構化輸出",
            subtitle="prompt 一個字都沒改，只多了 responseSchema 這個 API 參數",
            panels=[
                ...
                {"label": "產出", "tone": "gray",
                 "lines": [
                     ...
                     "    ⋯⋯共 4 條",
                     "  ],",
                     '  "seo_description": "晨光研選葉黃素膠囊⋯⋯",',
                     '  "hashtags": ["葉黃素", "游離型", "30mg"⋯⋯]',
                     "}",
                 ]},
            ],
            note="輸出從散文變成欄位，後面的程式才接得住。",
        ))
